export const HELP_SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    items: [
      ...
      { title: "线段", shortcut: "Alt+S", description: "可先选择两个或更多点后点击工具直接构造；也可依次点出两个点。" },
      { title: "直线", shortcut: "Alt+L", description: "可先选择两个或更多点后点击工具直接构造；也可依次点出两个点。" },
      { title: "射线", shortcut: "Alt+Y", description: "可先按顺序选择端点和方向点后点击工具直接构造；也可依次点出两点。" },
      { title: "圆", shortcut: "Alt+C", description: "可先选择圆心和圆上一点后点击工具直接构造；也可在“构造”菜单中用圆心和一条线段构造动态半径圆。" },
      { title: "过三点圆", shortcut: "Alt+O", description: "可先选择三个不共线的点后点击工具直接构造。系统保留动态圆心，但不显示或占用普通点的字母。" },
      { title: "中点", shortcut: "Alt+M", description: "可先选择两个点，或选择一条/多条线段后点击工具直接构造；也可点击线段或依次选择两点。" },
      { title: "中垂线", shortcut: "Alt+N", description: "可先选择两个点，或选择一条/多条线段后点击工具直接构造；也可依次点出两个位置。" },
  ...
  },
  {
    ...
    items: [
      ...
      { title: "变换菜单", description: "先标记中心、轴或反演圆，再选对象执行平移、旋转、缩放、轴对称、中心对称或反演；“重复最近变换”用于快速迭代。反演会动态使用所标记圆的圆心与半径，当前精确支持点、完整直线和圆。" },
      { title: "计算与坐标菜单", description: "菜单分为计算、坐标系和函数绘图三组。新建计算会打开非阻塞面板，可继续点击画布上的度量、参数或计算结果插入变量，并实时预览结果。" },
  ...
  },
  {
    ...
    items: [
      ...
      { title: "参数、度量与计算", description: "参数、度量值和计算结果都是可被其他表达式引用的动态数值对象。打开计算面板后，可点击变量按钮、属性检查器中的插入按钮，或直接点击画布上的数值对象，把变量插入当前光标位置。" },
